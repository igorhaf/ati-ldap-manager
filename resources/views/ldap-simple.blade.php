    ...
    <div id="app">
                ...
                <div class="flex justify-between items-center py-6">
                    ...
                    <div class="flex items-center space-x-3">
                        <button v-if="canManageUsers" @click="openCreateUserModal"
                            class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium transition-all duration-200 hover:shadow-lg hover:scale-105 flex items-center gap-2">
                        ...
                        <button v-if="isRoot" @click="showCreateOuModal = true"
                            class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-medium transition-all duration-200 hover:shadow-lg hover:scale-105 flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div v-if="canManageUsers || isRoot" class="relative" @click.self="showProfileMenu=false">
                            <button @click="showProfileMenu = !showProfileMenu"
                                class="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-xl font-medium transition-all duration-200 hover:shadow-lg backdrop-blur-sm flex items-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M5.121 17.804A13.937 13.937 0 0112 15c2.89 0 5.566.915 7.879 2.804M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                                @{{ userCn || userUid }}
                            ...
                            <div v-if="showProfileMenu"
                                ...
                                <a href="#" @click.prevent="openProfileModal"
            ...
            <div class="border-b border-gray-200 mb-8">
                <nav class="-mb-px inline-flex bg-white rounded-2xl p-2 gap-1 shadow-lg border border-gray-200">
                    <button @click="activeTab = 'users'"
                        ...
                        class="whitespace-nowrap px-6 py-3 rounded-xl transition-all duration-200 font-medium text-sm flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div v-if="activeTab === 'users'" class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="user in paginatedUsers" :key="user.uid"
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        ...
                                        <div v-if="isRoot" v-for="unit in user.organizationalUnits"
                                            ...
                                            :class="['inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-full mr-2 mb-1 border cursor-pointer select-none',
                                                ((typeof unit==='string' ? unit : unit.ou) === activeOuFilter) ?
                                                'bg-blue-600 text-white border-blue-600' :
                                                'bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border-blue-300/30 hover:brightness-90'
                                            ]">
                                            ...
                                            @{{ unit.ou ?? unit }}
                                            <span v-if="(unit.role ?? 'user') === 'admin'"
                                                class="ml-1 bg-orange-500 text-white text-xs px-1.5 py-0.5 rounded-full font-medium">Admin</span>
                                            <span v-if="unit.isActive !== undefined"
                                                :class="['ml-1 text-xs font-medium', unit.isActive ? 'text-green-600' :
                                                    'text-red-600'
                                                ]">
                                        ...
                                        <div v-else v-for="unit in user.organizationalUnits" :key="unit.ou ?? unit"
                                            ...
                                            :class="['inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-full mr-2 mb-1 border cursor-pointer select-none',
                                                (unit.role ?? 'user') === activeRoleFilter ? ((unit.role ??
                                                        'user') === 'admin' ?
                                                    'bg-orange-500 text-white border-orange-500' :
                                                    'bg-blue-600 text-white border-blue-600') : ((unit.role ??
                                                        'user') === 'admin' ?
                                                    'bg-gradient-to-r from-orange-100 to-orange-200 text-orange-800 border-orange-300/30 hover:brightness-90' :
                                                    'bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border-blue-300/30 hover:brightness-90'
                                                )
                                            ]">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                    ...
                                    <td class="px-6 py-4 text-sm"
                                        :class="user.uid === 'root' ? 'text-gray-500' : 'text-gray-900'">
                                        @{{ formatCpf(user.employeeNumber) }}</td>
                                    <td v-if="canManageUsers" class="px-6 py-4 text-sm font-medium">
                                        <template v-if="user.uid !== 'root'">
                                            <button @click="openEditUserModal(user)"
                                                class="inline-flex items-center gap-1 text-blue-600 hover:text-blue-900 mr-4 transition-colors">
                                            ...
                                            <button @click="deleteUser(user.uid)"
                                                class="inline-flex items-center gap-1 text-red-600 hover:text-red-900 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                    ...
                    <div class="flex justify-center items-center mt-4 mb-8 space-x-1" v-if="totalUsersPages > 1">
                        ...
                        <button v-for="n in pageNumbers(totalUsersPages)" :key="'u' + n"
                            @click="setPage('users',n)"
                            :class="['px-3 py-1 rounded-full', usersPage === n ? 'bg-blue-600 text-white' :
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <div class="flex justify-center items-center mt-4 mb-8 space-x-1" v-if="totalOusPages > 1">
                        <button @click="prevPage('ous')" :disabled="ousPage === 1"
                            class="px-2 py-1 border rounded disabled:opacity-50">«</button>
                        <button v-for="n in pageNumbers(totalOusPages)" :key="'o' + n"
                            @click="setPage('ous',n)"
                            :class="['px-3 py-1 rounded-full', ousPage === n ? 'bg-blue-600 text-white' :
                                'border border-gray-300 bg-white hover:bg-gray-100'
                            ]">@{{ n }}</button>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
                            ...
                            <input v-model="logFilters.action" type="text" placeholder="Filtrar por Ação"
                                class="px-3 py-2 border rounded-md">
                            <input v-model="logFilters.target" type="text"
                            ...
                            <input v-model="logFilters.cpf" @input="maskCpfFilter" inputmode="numeric"
                                type="text" placeholder="Filtrar por CPF" class="px-3 py-2 border rounded-md"
                                title="CPF">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Usuário
                                        afetado</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        Organização</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        Resultado</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Quando
                                    </th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                        Descrição</th>
                                </tr>
                    ...
                    <div class="flex justify-center items-center mt-4 mb-8 space-x-1" v-if="totalLogPages > 1">
                        <button @click="prevPage('logs')" :disabled="logsPage === 1"
                            class="px-2 py-1 border rounded disabled:opacity-50">«</button>
                        <button v-for="n in pageNumbers(totalLogPages)" :key="'l' + n"
                            @click="setPage('logs',n)"
                            :class="['px-3 py-1 rounded-full', logsPage === n ? 'bg-blue-600 text-white' :
                                'border border-gray-300 bg-white hover:bg-gray-100'
                            ]">@{{ n }}</button>
        ...
        <transition name="slide-in">
            <div v-if="showRightDrawer" class="fixed inset-0 z-50" @click="closeLogDrawer">
                <div class="fixed top-0 right-0 h-full w-full md:w-96 lg:w-1/3 xl:w-1/4 bg-white shadow-2xl border-l border-gray-200 overflow-y-auto"
                    @click.stop>
                    ...
                    <div class="p-4 space-y-4" v-if="selectedLog">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="bg-gray-50 rounded-lg p-3">
                                ...
                                <div class="text-sm"
                                    :class="selectedLog.result === 'Sucesso' ? 'text-green-700 font-medium' :
                    ...
                    <form @submit.prevent="createUser" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="newUser.givenName" type="text" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Sobrenome</label>
                                <input v-model="newUser.sn" type="text" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">UID (Login)</label>
                                <input v-model="newUser.uid" @input="uidManuallyEdited = true" type="text"
                                    required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">CPF</label>
                                <input v-model="newUser.employeeNumber" @input="maskCpf('new')" type="text"
                            ...
                            <div class="space-y-2">
                                <div v-for="(unit, index) in newUser.organizationalUnits" :key="index"
                                    ...
                                    <label class="inline-flex items-center cursor-pointer select-none ml-2">
                                        <input type="checkbox" v-model="newUser.organizationalUnits[index].isActive"
                                            class="sr-only peer">
                                        <div
                        ...
                        <div class="flex justify-end space-x-4 pt-8 border-t border-gray-200">
                            <button @click="showCreateUserModal = false" type="button"
                                class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 font-medium transition-colors">Cancelar</button>
                            <button type="submit"
                                class="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium transition-colors hover:shadow-lg">Criar
                                Usuário</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="w-11/12 md:w-3/4 lg:w-1/2 bg-white rounded-2xl shadow-2xl border border-gray-100">
                <div class="p-8">
                    <div class="flex justify-between items-center mb-8">
                        ...
                        <button @click="showCreateOuModal = false"
                            class="text-gray-400 hover:text-gray-600 p-2 rounded-lg hover:bg-gray-100 transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                    ...
                    <form @submit.prevent="createOu" class="space-y-4">
                        ...
                        <div class="flex justify-end space-x-4 pt-8 border-t border-gray-200">
                            <button @click="showCreateOuModal = false" type="button"
                                class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 font-medium transition-colors">Cancelar</button>
                            <button type="submit"
                                class="px-6 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 font-medium transition-colors hover:shadow-lg">Criar
                                Unidade Organizacional</button>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <form @submit.prevent="updateUser" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" v-model="editUser.uid"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">CPF</label>
                                <input type="text" v-model="editUser.employeeNumber" @input="maskCpf('edit')"
                                    inputmode="numeric"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nome</label>
                                <input v-model="editUser.givenName" type="text"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    :class="editUser.isRootUser ? 'bg-gray-100' : ''" :disabled="editUser.isRootUser">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Sobrenome</label>
                                <input v-model="editUser.sn" type="text"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    :class="editUser.isRootUser ? 'bg-gray-100' : ''" :disabled="editUser.isRootUser">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input v-model="editUser.description" type="email" id="edit-redirect-mail"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="Ex: meuemail@gmail.com"
                                    :class="editUser.isRootUser ? 'bg-gray-100' : ''" :disabled="editUser.isRootUser">
                            </div>
                        ...
                        <div class="flex justify-end space-x-4 pt-8 border-t border-gray-200">
                            <button @click="showEditUserModal = false" type="button"
                                class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 font-medium transition-colors">Cancelar</button>
                            <button type="submit"
                                class="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium transition-colors hover:shadow-lg">Salvar
                                Alterações</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-8">
                    ...
                    <form @submit.prevent="saveProfile" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Sobrenome</label>
                                <input v-model="profile.sn" type="text"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    :class="isRoot ? 'bg-gray-100' : ''" :disabled="isRoot">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">CPF</label>
                                <input v-model="profile.employeeNumber" @input="maskCpfProfile" inputmode="numeric"
                                    type="text"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    placeholder="000.000.000-00" :class="isRoot ? 'bg-gray-100' : ''"
                                    :disabled="isRoot">
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end space-x-4 pt-8 border-t border-gray-200">
                            <button @click="closeProfileModal" type="button"
                                class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 font-medium transition-colors">Cancelar</button>
                            <button type="submit"
                                class="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium transition-colors hover:shadow-lg">Salvar</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div v-if="showDeleteUserModal"
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">Tem certeza que deseja excluir o usuário
        ...
        <div v-if="showEditOuModal"
            class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div
                class="w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-2xl border border-gray-100">
                <div class="p-8">
                    <div class="flex justify-between items-center mb-8">
                        <h3 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                            <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor"
                        ...
                        <button @click="showEditOuModal = false"
                            class="text-gray-400 hover:text-gray-600 p-2 rounded-lg hover:bg-gray-100 transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M6 18L18 6M6 6l12 12" />
                    ...
                    <form @submit.prevent="updateOu" class="space-y-4">
                        ...
                        <div class="flex justify-end space-x-4 pt-8 border-t border-gray-200">
                            <button @click="showEditOuModal = false" type="button"
                                class="px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 font-medium transition-colors">Cancelar</button>
                            <button type="submit"
                                class="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium transition-colors hover:shadow-lg">Salvar
                                Alterações</button>
            ...
            createApp({
                data() {
                    return {
                        ...
                        defaultEmailDomain: (window.USER_MAIL && window.USER_MAIL.includes('@')) ? window.USER_MAIL
                            .split('@')[1] : '',
                        newUser: {
                            uid: '',
                            givenName: '',
                ...
                computed: {
                    ...
                    filteredUsers() {
                        ...
                        if (this.activeOuFilter && this.isRoot) {
                            list = list.filter(u => {
                                return (u.organizationalUnits || []).some(unit => {
                                    const ouName = typeof unit === 'string' ? unit : (unit.ou ??
                                        unit);
                        ...
                        if (this.activeRoleFilter && !this.isRoot) {
                            list = list.filter(u => {
                                return (u.organizationalUnits || []).some(unit => {
                                    const role = typeof unit === 'string' ? 'user' : (unit.role ??
                                        'user');
                        ...
                        return list.filter(user => {
                            ...
                            return uid.includes(term) || fullName.includes(term) || cpf.includes(term) ||
                                mail.includes(term);
                        });
                    },
                    // Paginação usuários
                    ...
                    totalOusPages() {
                        return Math.ceil(this.filteredOus.length / this.itemsPerPage) || 1;
                    },
                    ...
                    filteredLogs() {
                        ...
                        return this.logs.filter(l => {
                            ...
                            if (f.cpf) {
                                ...
                                const hasMatch = candidates.some(c => c && c.length === 11 && c ===
                                    targetCpf);
                ...
                watch: {
                    'newUser.givenName'(val) {
                        this.suggestUidAndMail();
                    },
                    'newUser.sn'(val) {
                        this.suggestUidAndMail();
                    },
                ...
                methods: {
                    ...
                    suggestUidAndMail() {
                        ...
                        const suggestedUser = [norm(given), norm(sn)].filter(Boolean).join('.');

                        if (!this.uidManuallyEdited) {
                        ...
                        try {
                            ...
                            if (data.success) {
                                ...
                                this.users = data.data.map(u => {
                                    if (Array.isArray(u.organizationalUnits) && typeof u
                                        .organizationalUnits[0] === 'string') {
                                        u.organizationalUnits = u.organizationalUnits.map(o => ({
                                            ou: o,
                                            role: 'user'
                                        }));
                                    }
                    ...
                    openEditUserModal(user) {
                        ...
                        if (Array.isArray(user.organizationalUnits)) {
                            if (typeof user.organizationalUnits[0] === 'string') {
                                this.editUser.organizationalUnits = user.organizationalUnits.map(ou => ({
                                    ou,
                                    role: 'user'
                                }));
                            } else {
                                this.editUser.organizationalUnits = JSON.parse(JSON.stringify(user
                                    .organizationalUnits));
                            }
                        } else {
                            this.editUser.organizationalUnits = [{
                                ou: '',
                                role: 'user'
                            }];
                        }
                    ...
                    async updateUser() {
                        try {
                            ...
                            // Validar e-mail de redirecionamento (se informado)
                            if (userData.description && !/^([^\s@]+)@([^\s@]+)\.[^\s@]+$/.test(userData
                                    .description)) {
                    ...
                    async createUser() {
                        try {
                            ...
                            if (this.isOuAdmin) {
                                ...
                                if (!userData.organizationalUnits || userData.organizationalUnits.length ===
                                    0 ||
                                    !userData.organizationalUnits[0].ou || userData.organizationalUnits[0].ou
                                    .trim() === '') {
                        ...
                        if (this.isOuAdmin) {
                            ...
                            if (!this.adminOu || this.adminOu.trim() === '') {
                                ...
                                this.loadUsers().then(async () => {
                                    ...
                                    this.showNotification('Erro ao carregar dados. Recarregue a página.',
                                        'error');
                                });
                            } else {
                                this.showCreateUserModal = true;
                    ...
                    async getAdminOu() {
                        try {
                            ...
                            const adminOuEntry = currentUser.organizationalUnits.find(unit => {
                                ...
                                console.log(
                                    `  📍 Verificando OU: ${typeof unit === 'string' ? unit : unit.ou}, Role: ${role}`
                                );
                                return role === 'admin';
                            });
                    ...
                    pageNumbers(total) {
                        const current = this.activeTab === 'users' ? this.usersPage : (this.activeTab ===
                            'organizational-units' ? this.ousPage : this.logsPage);
                        const delta = 2;
                        const range = [];
                    ...
                    maskCpf(context) {
                        ...
                        if (digits.length > 9) formatted = digits.replace(/(\d{3})(\d{3})(\d{3})(\d{0,2})/, (m, a,
                            b, c, d) => d ? `${a}.${b}.${c}-${d}` : `${a}.${b}.${c}`);
                        else if (digits.length > 6) formatted = digits.replace(/(\d{3})(\d{3})(\d{0,3})/, (m, a, b,
                            c) => c ? `${a}.${b}.${c}` : `${a}.${b}`);
                        else if (digits.length > 3) formatted = digits.replace(/(\d{3})(\d{0,3})/, (m, a, b) => b ?
                            `${a}.${b}` : `${a}`);
                    ...
                    async saveProfile() {
                        ...
                        try {
                            ...
                            const resp = await fetch(
                                `/api/ldap/users/${encodeURIComponent(this.profile.uid)}`, {
                                    method: 'PUT',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')
                            ...
                            if (this.profilePassword) {
                                const respPwd = await fetch(
                                    `/api/ldap/users/${encodeURIComponent(this.profile.uid)}/password`, {
                                        method: 'PUT',
                                        headers: {
                                            'Content-Type': 'application/json',
